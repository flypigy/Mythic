...
extension Wine {
    ...
    final class DXVK {
        ...
        private static let dxvkDLLs = ["d3d10core.dll", "d3d11.dll", "dxgi.dll"]

        /// The DLLs that DXMT overrides (includes winemetal.dll, which DXMT requires at runtime).
        private static let dxmtDLLs = ["d3d10core.dll", "d3d11.dll", "dxgi.dll", "winemetal.dll"]
        ...
        static func install(toContainerAtURL containerURL: URL) async throws {
            try await install(from: Engine.directory.appending(path: "dxvk"), dlls: dxvkDLLs, toContainerAtURL: containerURL)
        }
        ...
        static func installDXMT(toContainerAtURL containerURL: URL) async throws {
            try await install(from: Engine.directory.appending(path: "dxmt"), dlls: dxmtDLLs, toContainerAtURL: containerURL)
        }
        ...
        ///   - dlls: The list of DLL filenames to copy (e.g. d3d10core.dll, d3d11.dll, ...).
        ///   - containerURL: The Wine container (prefix) root URL.
        private static func install(from componentDirectory: URL,
                                    dlls: [String],
                                    toContainerAtURL containerURL: URL) async throws {
            try Wine.killAll(at: containerURL)

            // remove existing DLLs (both x64 and x32) so the override takes effect cleanly
